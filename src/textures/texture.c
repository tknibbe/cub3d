...
static void	draw_wall(t_ray ray, t_game *game, int x)
{
	...
	ray.texpos = (ray.wall_start - HALF_HEIGHT + game->wall_off + ray.wall_height / 2) * ray.step;
...
void	line_to_buffer(t_game *game, t_ray ray_vars, int x)
{
	...
	ray_vars.wall_start = -ray_vars.wall_height / 2 + HALF_HEIGHT - game->wall_off;
	if (ray_vars.wall_start < 0)
		ray_vars.wall_start = 0;
	ray_vars.wall_end = ray_vars.wall_height / 2 + HALF_HEIGHT - game->wall_off;
